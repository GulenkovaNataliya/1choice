...
export default async function EditPropertyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: property, error }, areas, badges] = await Promise.all([
    supabase
      .from("properties")
      .select(
        "id,property_code,title,slug,category,subtype,transaction_type,price_eur,location,location_text,summary,description,size_sqm,bedrooms,bathrooms,floor,year_built,year_renovated,building_condition,energy_class,heating_type,custom_heating,cooling_type,custom_cooling,heating_system,heating_fuel,heating_features,cooling_system,fireplace,elevator,security_door,alarm_system,video_doorphone,smart_home,satellite_tv,internet_ready,wardrobe_room,sea_view,mountain_view,balcony,veranda,awnings,garden,pool,parking,parking_spaces,parking_type,parking_level,parking_area_sqm,parking_suitable_for,parking_features,exposure,jacuzzi,close_to_beaches,panoramic_view,acropolis_view,private_roof_terrace,loft,internal_staircase,barbeque,home_cinema,smoke_detection,total_property_area_sqm,total_building_floors,number_of_levels,level_details,frames_type,single_glazing,double_glazing,triple_glazing,mosquito_screens,thermal_insulation,sound_insulation,blinds,electric_shutters,flooring_type,living_rooms,kitchens,storage_rooms,wc,versatile_rooms,furnished,custom_furnished,cover_image_url,gallery_image_urls,youtube_video_url,virtual_tour_url,latitude,longitude,approximate_location,address,show_address,is_golden_visa,featured,private_collection,publish_1choice,publish_deals,status,agent_notes,custom_badge,custom_badge_color,land_area_sqm,building_coefficient,coverage_ratio,frontage_m,remaining_buildable_area_sqm,town_planning_status,land_slope,land_features,electricity"
      )
      .eq("id", id)
      .single(),
    fetchActiveAreas(),
  ...
  const initialValues = {
    ...
    single_glazing: (property as { single_glazing?: boolean | null }).single_glazing ?? false,
    ...
    blinds: (property as { blinds?: boolean | null }).blinds ?? false,
    electric_shutters: (property as { electric_shutters?: boolean | null }).electric_shutters ?? false,
    ...
    // Land / Plot
    land_area_sqm: (property as { land_area_sqm?: number | null }).land_area_sqm != null ? String((property as { land_area_sqm?: number | null }).land_area_sqm) : "",
    building_coefficient: (property as { building_coefficient?: number | null }).building_coefficient != null ? String((property as { building_coefficient?: number | null }).building_coefficient) : "",
    coverage_ratio: (property as { coverage_ratio?: number | null }).coverage_ratio != null ? String((property as { coverage_ratio?: number | null }).coverage_ratio) : "",
    frontage_m: (property as { frontage_m?: number | null }).frontage_m != null ? String((property as { frontage_m?: number | null }).frontage_m) : "",
    remaining_buildable_area_sqm: (property as { remaining_buildable_area_sqm?: number | null }).remaining_buildable_area_sqm != null ? String((property as { remaining_buildable_area_sqm?: number | null }).remaining_buildable_area_sqm) : "",
    town_planning_status: (property as { town_planning_status?: string | null }).town_planning_status ?? "",
    land_slope: (property as { land_slope?: string | null }).land_slope ?? "",
    land_features:   (property as { land_features?: string[] | null }).land_features ?? [],
    electricity:     (property as { electricity?: string[] | null }).electricity ?? [],
  };
